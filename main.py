...
def calc_cost(AL, Y_real, buffer, lamda=0, parameters=None):
    # lamda and parameters are for future functionality
    m = Y_real.shape[1]
    # print("m=", m)
    cost = (-1 / m) * (
        np.sum(np.multiply(Y_real, np.log(AL + buffer)) + np.multiply(1 - Y_real, np.log(1 - AL + buffer))))
...
def backprop(AL, Y, buffer, caches, parameters):
    grads = {}
    L = len(caches) // 2
    m = AL.shape[1]
...
def train_model(X, Y_real, buffer, parameters, imgname="test"):
    costs = []
    for i in range(num_iter):
        Y, cache = forward_prop(X, parameters)
        cost = calc_cost(Y, Y_real, buffer)
        costs.append(cost)
        grads = backprop(Y, Y_real, buffer, cache, parameters)
...
if __name__ == "__main__":
    ...
    cost = train_model(X, Y_real, buffer, parameters, imgname=report_name + str(idrep))
